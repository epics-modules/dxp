...
/* eppPort stores the file number for parport, only used for the X10P/G200 */
int eppPort;
...
static int DxpInitEPP(int base)
{
   ...
   eppPort = open (name, O_RDWR);
   if (eppPort == -1) {
      sprintf(error_string,"Error opening EPP port"); 
      dxp_md_log_error("DxpInitEpp",error_string,-1);
      return -1;
   }
   status = ioctl(eppPort, PPCLAIM);
   if (status != 0) {
      sprintf(error_string,"Error claiming EPP port"); 
      dxp_md_log_error("DxpInitEpp",error_string,-1);
      close (eppPort);
      return -1;
   }
   mode = IEEE1284_MODE_EPP;
   status = ioctl (eppPort, PPSETMODE, &mode);
   if (status != 0) {
      sprintf(error_string,"Error setting EPP port mode to IEEE1284_MODE_EPP"); 
      dxp_md_log_error("DxpInitEpp",error_string,-1);
      return -1;
   }
...
static int set_addr(unsigned short d) {
        unsigned char   data;
        int status, mode;
...
static int DxpWriteWord(unsigned short addr, unsigned short data) 
{
        ...
        sprintf(error_string, "addr==%#x, data=%#x ", addr, data);
        dxp_md_log_debug("DxpWriteWord", error_string);
        if (addr < 0x4000)
           return(-1);
        if (set_addr(addr))
           return(-1);
        c0 = data&0xFF;
        status = write(eppPort, &c0, 1);
        if (status != 1) return(-1);
        c0 = (data>>8)&0xFF;
        status = write(eppPort, &c0, 1);
        if (status != 1) return(-1);
...
static int DxpReadWord(unsigned short addr, unsigned short *data)
{
        ...
        if (set_addr(addr))
           return(-1);
        status = read(eppPort, &c0, 1);
        if (status != 1) return(-1);
        status = read(eppPort, &c1, 1);
        if (status != 1) return(-1);
        *data = c0|(c1<<8);
        sprintf(error_string, "addr==%#x, data=%#x ", addr, *data);
        dxp_md_log_debug("DxpReadWord", error_string);
        return(0);
}
...
static int DxpWriteBlock(unsigned short addr, unsigned short *data, int len)
{
        ...
        if (set_addr(addr))
           return(-1);
        sprintf(error_string, "addr==%#x, data=%#x, len=%d", addr, *data, len);
        dxp_md_log_debug("DxpWriteBlock", error_string);
        /* Need to worry about swapping byte order here !!! */
        status = write(eppPort, data, 2*len);
        if (status != 2*len) return(-1);
        return(0);
}
...
static int DxpWriteBlocklong(unsigned short addr, unsigned long *data, int len)
{
        ...
        if (set_addr(addr))
            return(-1);
        sprintf(error_string, "addr==%#x, data=%#lx, len=%d", addr, *data, len);
        dxp_md_log_debug("DxpWriteBlock", error_string);
        /* Need to worry about swapping byte order here !!! */
        status = write(eppPort, data, 4*len);
        if (status != 4*len) return(-1);
        return(0);
}
...
static int DxpReadBlock(unsigned short addr, unsigned short *data, int len)
{
        ...
        if (set_addr(addr))
            return(-1);
        /* Need to worry about swapping byte order here !!! */
        status = read(eppPort, data, 2*len);
        sprintf(error_string, "addr==%#x, data=%#x, len=%d", addr, *data, len);
        dxp_md_log_debug("DxpReadBlock", error_string);
        if (status != 2*len) return(-1);
        return(0);
}
...
static int DxpReadBlocklong(unsigned short addr, unsigned long *data, int len)
{
        ...
        if (set_addr(addr))
            return(-1);
        /* Need to worry about swapping byte order here !!! */
        status = read(eppPort, data, 4*len);
        sprintf(error_string, "addr==%#x, data=%#lx, len=%d", addr, *data, len);
        dxp_md_log_debug("DxpReadBlockLong", error_string);
        if (status != 4*len) return(-1);
        return(0);
