 ...
 * long integer */
typedef long (__stdcall *MYDLLFUNC)(short*,short,long,short,short*);
...
/* Free the CAMAC library if already allocated */
	if (camLibrary!=NULL) {
		FreeLibrary(camLibrary);
		camLibrary = NULL;
		camTransferFunc = NULL;
	}

/* Now make the link to the DLL containing the CAMAC routines */
    camLibrary = LoadLibrary(dllname);
    if (camLibrary == NULL){
        status = DXP_OPEN_FILE;
        sprintf(error_string,"LoadLibrary(%s) Failed", dllname);
        dxp_md_error("dxp_md_initialize",error_string,&status);
		return status;
	}
	
/* Now make the link to the function to access the CAMAC crate */

    camTransferFunc = (MYDLLFUNC) GetProcAddress (camLibrary,"_camxfr@20");
    if (camTransferFunc == NULL) {
        status = DXP_OPEN_FILE;
        sprintf(error_string,"GetProcAddress(%s,camxfr@20) Failed", dllname);
        dxp_md_error("dxp_md_initialize",error_string,&status);
        FreeLibrary(camLibrary);
		camLibrary = NULL;
        return status;
...
/* prelimenary definitions used by the DLL routine */
    nbytes=2*(*length);
    camadr[0] = (short) branch[*camChan];
    camadr[1] = (short) crate[*camChan];
    camadr[2] = (short) station[*camChan];
    camadr[3] = (short) *address;

/* Now perform the read or write operation using the routine */

	lstatus = (*camTransferFunc)(camadr, func, nbytes, mode, (short*) data);
